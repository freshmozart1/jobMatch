...
- A running scrape had no user-facing way out. `MatchPage.vue` already held a `scrapeAbortController` and already aborted it in two places — at the top of `fetchJobs()` when a new search supersedes the old one, and in `onUnmounted()` — but neither was reachable as a deliberate action: the loading state rendered a single line of text (`<p class="match-page__status">Loading jobs...</p>`) and nothing to interact with. A scrape observed stalling for 7+ minutes therefore left closing the tab as the only escape, which works (the backend logs "LinkedIn scrape aborted: client disconnected") but costs the entire session — keywords survive in `localStorage`, the deck does not, and the next load starts a brand-new scrape. Added `src/components/CancelScrapeButton.vue`, a pill-outline "Cancel" button following the project's claude.ai/design reference, and rendered it in **both** loading states: the full-area "Loading jobs..." state in `MatchPage.vue`, and `JobCardStack.vue`'s "Loading more jobs..." state, which is what the user is looking at after swiping through a partial deck while the scrape is still running — the stack re-emits the button's click as its own `cancel` event. `JobCardStack.vue` imports the button by relative path on purpose: `src/components/index.ts` re-exports it, so routing through the barrel would close an import cycle. Clicking it runs `cancelScrape()`, which sets `scrapeCancelled`, clears `isLoading` immediately rather than waiting for the aborted stream to reject (a stream that has already been fully buffered drains to `done` and never rejects at all), and aborts the controller. `fetchJobs()`'s `catch` now checks `signal.aborted` before assigning `errorMessage`, so neither a user-requested stop nor a superseded scrape surfaces as a failure while a genuine fetch rejection still does. The partial deck survives untouched, since `jobs.value` is only reset at the _start_ of `fetchJobs()`, and `emptyLabel` reports the real reason the deck ran out: "Search stopped" after a cancel, taking precedence over the match filter's "No jobs at or above N% match" when nothing was scraped at all, since a threshold that never saw a job cannot be what emptied the deck. `cancelScrape()` also clears `lastFetchedParams`, so reopening and closing the search sheet re-runs the _same_ search instead of `searchParamsChanged()` finding nothing changed and silently doing nothing — without that, a cancelled search is exactly the dead end the control exists to prevent. The initial-load branch of the template now wins over the error branch, so an error frame arriving mid-scrape can no longer re-trap the user on a screen with no way to stop it: the warning line renders above "Loading jobs..." inside `.match-status-fill`, which became a column flex container, and `MatchPage.css`'s pulse was narrowed to skip that warning (a settled fact, not a sign of ongoing work), with the then-unused `.match-page__status--loading` rule deleted. `fetchJobs()`'s per-event body was extracted into `applyScrapeEvent()` to keep the stream loop readable under the added branch. Covered by nine new `MatchPage.spec.ts` tests (cancel control present beside the loading text, the signal actually aborting, no error state, the "Search stopped" settle, the partial deck surviving a cancel, a real failure still reported as an error, the same search re-running after a cancel, the scrape staying cancellable after an early error frame, and the stopped label winning over the match filter) and four in `JobCardStack.spec.ts`; the shared SSE test double gained an `abort()` that errors the body stream the way a real aborted fetch does, and `eslint.config.ts` learned `expectSearchStopped` as a `vitest/expect-expect` assertion function (closes #72).
