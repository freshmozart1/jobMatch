...
const props = defineProps<{ job: ScrapedJob; dragOffsetX?: number; isDragging?: boolean }>()
...
<template>
  <article
    ...
    :style="{ transform: `translateX(${dragOffsetX ?? 0}px) rotate(${rotation}deg)` }"
  >
    <div class="stamp stamp--like" :style="{ opacity: direction === 'right' ? progress : 0 }">Like</div>
