...
describe('MatchFilterBar', () => {
    it('reads "Show all jobs" and disables the input when off', () => {
        ...
        expect(wrapper.find('.match-filter__label').text()).toBe(
            'Show all jobs',
        );
        expect(
            wrapper.find('.match-filter__switch').attributes('aria-checked'),
        ).toBe('false');
        expect(
            wrapper.find('.match-filter__num input').attributes('disabled'),
        ).toBeDefined();
    });
