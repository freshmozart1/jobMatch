...
describe('SearchPage', () => {
    ...
    it('generates a deterministic tag color for the same keyword', () => {
        ...
        expect(w1.find('.se-tag').attributes('style')).toBe(
            w2.find('.se-tag').attributes('style'),
        );
    });
    ...
    it('saves city to localStorage on city input', async () => {
        ...
        expect(window.localStorage.getItem('jobmatch.searchcity')).toBe(
            'Hamburg',
        );
    });
    ...
    it('strips non-numeric characters from distance input and saves to localStorage', async () => {
        ...
        expect(window.localStorage.getItem('jobmatch.searchdistance')).toBe(
            '12',
        );
    ...
    it('renders the three date posted options', () => {
        const wrapper = mount(SearchPage, { props: { keywords: [] } });
