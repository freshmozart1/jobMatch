...
const indicator = '[data-testid="cosineSimilarityIndicator"]'
...
describe('JobCardCosineSimilarity', () => {
  ...
  describe('clamped value (aria-valuenow)', () => {
    ...
    it('keeps an in-range value unchanged', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.75 } })
  ...
  describe('percent display', () => {
    it('rounds 0.004 down to 0%', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.004 } })
      expect(wrapper.find('.cosine-indicator__value').text()).toBe('0%')
    })

    it('rounds 0.005 up to 1%', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.005 } })
      expect(wrapper.find('.cosine-indicator__value').text()).toBe('1%')
    })

    it('rounds 0.874 to 87%', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.874 } })
      expect(wrapper.find('.cosine-indicator__value').text()).toBe('87%')
    })
  })

  describe('cosineSimilarityLevel labels', () => {
    it('shows "poor match" for values below 0.25', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0 } })
      expect(wrapper.find(indicator).attributes('aria-valuetext')).toContain('poor match')
    })

    it('shows "weak match" at exactly 0.25', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.25 } })
      expect(wrapper.find(indicator).attributes('aria-valuetext')).toContain('weak match')
    })

    it('shows "moderate match" at exactly 0.5', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.5 } })
      expect(wrapper.find(indicator).attributes('aria-valuetext')).toContain('moderate match')
    })

    it('shows "strong match" at exactly 0.75', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.75 } })
      expect(wrapper.find(indicator).attributes('aria-valuetext')).toContain('strong match')
    })
  ...
  describe('ARIA attributes', () => {
    ...
    it('aria-valuetext contains the percentage and level label', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.87 } })
  ...
  describe('fillColor', () => {
    it('produces an oklch() color string', () => {
      const wrapper = mount(JobCardCosineSimilarity, { props: { value: 0.5 } })
      expect(wrapper.find('.cosine-indicator__fill').attributes('style')).toContain('oklch(')
    })
    ...
    it('produces different colors at 0 and 1 (hue interpolates)', () => {
      ...
      const styleLow = low.find('.cosine-indicator__fill').attributes('style') ?? ''
      const styleHigh = high.find('.cosine-indicator__fill').attributes('style') ?? ''
