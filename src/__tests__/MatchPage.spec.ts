  ...
  return vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.endsWith("/scrape/linkedin/job-links")) {
      return jobLinksHandler
        ? jobLinksHandler()
        : createJsonResponse(jobLinksByKeyword);
    }
...
async function waitForJobPageCallCount(
...
describe("MatchPage", () => {
  ...
  it("limits concurrent job page scrape requests to 2 at a time", async () => {
    ...
    const fetchMock = buildFetchMock(
      async (jobUrl) => {
        ...
        if (!deferred)
          return createJsonResponse({ error: "Unknown URL" }, { status: 404 });
    ...
    const jobPageUrls = fetchMock.mock.calls
      ...
      .map(
        (args) =>
          JSON.parse((args[1] as RequestInit).body!.toString()).url as string,
      );
    expect(jobPageUrls).not.toContain(thirdJob.sourceUrl);

    // Resolve the first — the third must now start.
    deferredResponses
      .get(testJobs[0]!.sourceUrl)!
      .resolve(createJsonResponse(testJobs[0]));
    await waitForJobPageCallCount(fetchMock, 3);

    // Clean up remaining deferred promises.
    deferredResponses
      .get(testJobs[1]!.sourceUrl)!
      .resolve(createJsonResponse(testJobs[1]));
    deferredResponses
      .get(thirdJob.sourceUrl)!
      .resolve(createJsonResponse(thirdJob));
  });
  ...
  it("renders a scraped job while later job page requests are still pending", async () => {
    ...
    expect(wrapper.find(".match-page__status--loading").text()).toBe(
      "Loading more jobs...",
    );
  ...
  it("renders the mobile layout anchors for the card stack and sticky controls", async () => {
    ...
    expect(wrapper.find(".job-card-stack__current .job-card").exists()).toBe(
      true,
    );
    expect(
  ...
  it("scales the next card up while dragging", async () => {
    const wrapper = await mountLoadedMatchPage();
    expect(wrapper.find(".job-card-stack__next").attributes("style")).toContain(
      "scale(0.92)",
    );
